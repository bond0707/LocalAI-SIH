<script lang="ts">
	...
	const chatCompletionEventHandler = async (data, message, chatId) => {
		const { id, done, choices, content, output, sources, selected_model_id, routing_info, error, usage } = data;
		...
		if (routing_info) {
			message.routingInfo = routing_info;
		}
		...
		try {
			...
			if (res && res.ok && res.body && generating) {
				...
				for await (const update of textStream) {
					const { value, done, sources, error, usage, selectedModelId, routingInfo } = update;
					if (selectedModelId) {
